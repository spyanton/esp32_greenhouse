...
void sendMessage(String message){
  ...
  String url = "http://api.callmebot.com/whatsapp.php?phone="+urlEncode(phone_number)+"&text="+urlEncode(message)+"&apikey="+urlEncode(api_key);
  WiFiClient client;    
  HTTPClient http;
...
void loop(void) {
  ...
  if(temp > 30 || temp < 3){
    if(trigger == true){ 
      ...
      sendMessage("WARNING! Temperature is: ");
      delay(5000);
      sendMessage(s);
      delay(5000);
      trigger = false;
      ...
      String s = String(temp);
      sendMessage("Temperature has reached normal parameters: ");
      delay(5000);
      sendMessage(s);
      delay(5000);
      trigger = true;
